...
class Expression(abc.ABC, typing.Generic[T]):
    ...
    def evaluate(self) -> T:
        ...
        if callable(self._function):
            if (expected_arity == children_arity):
                ...
                raise TypeError(self._function, expected_arity, children_arity)
    ...
    def copy(self) -> Self:
        ...
        children_copies: Tuple[Expression[T], ...] =\
            tuple(x.copy() for x in self.children)
    ...
    def __str__(self) -> str:
        ...
        my_name: str = self._function.__name__\
            if callable(self._function) else str(self._function)
...
class ExpressionFactory(typing.Generic[T]):
    def __init__(self, functions: Tuple[Callable[..., T], ...]):
        # Build a pool of functions, as a dictionary where each index
        #   corresponds to an arity, and each value is a list of functions
        #   (or terminals!) with that arity.
    ...
    def poll_function(self,
                      nullary_ratio: Optional[float] = None) -> Callable[..., T]:
        self.cost_budget()
        ...
        if (nullary_ratio is None):
            ...
            if (random.random() < nullary_ratio):
                ...
                choice_poll = choice(
                    [self.function_pool[x] for x in self.function_pool.keys() if x != 0])
...
class ProgramFactory(Generic[T]):
    def __init__(self, functions: Tuple[Callable[..., T], ...], arity):
        self.arity = arity
        self._symbol_count = 0  # only used to keep track of symbol names.
        # Does not relate to arity
    ...
    def build(self, depth: int,
              budget: int,
              unary_weight: Optional[float] = None) -> Program:
        return Program(self.exprfactory.build(depth, budget, unary_weight),
                       self.symbol_deposit,
                       self)
...
class ProgramArityMismatchError(Exception):
    def __init__(self, expected: Optional[int], given: Optional[int]):
        super().__init__(f"The program is expecting {expected} arguments,"
                         f"only {given} are given.")
...
class ProgramCrossoverVariator(Variator[Program[float]]):
    ...
    def crossover(self,
        ...
        expression_internal_nodes_from_root_1 =\
            [x for x in expression_nodes_from_root_1 if len(x.children) > 0]
        expression_internal_nodes_from_root_2 =\
            [x for x in expression_nodes_from_root_2 if len(x.children) > 0]

        # If both expression trees have valid internal nodes,
        if (len(expression_internal_nodes_from_root_1) >=
                1 and len(expression_internal_nodes_from_root_2) >= 1):
            expression_node_from_root_1_to_swap =\
                random.choice(expression_internal_nodes_from_root_1)
            expression_node_from_root_2_to_swap =\
                random.choice(expression_internal_nodes_from_root_2)
            self.__class__.swap_children(
